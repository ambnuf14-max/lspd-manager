...
class FeedbackModal(discord.ui.Modal, title='Получение роли'):
    ...
    forum = discord.ui.TextInput(
        ...
        placeholder=f'Проверьте, указан ли в профиле дискорд.',
        required=True,
        max_length=100,
    )
    ...
    vk = discord.ui.TextInput(
        label='Ваш ВКонтакте:',
        style=discord.TextStyle.short,
        placeholder=f'https://vk.com/id1',
        required=True,
        max_length=100,
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # Создаем Embed
        embed = discord.Embed(title=f"Новый запрос",
    ...
    async def on_error(self, interaction: discord.Interaction, error: Exception) -> None:
        await interaction.response.send_message('Упс! Что-то пошло не так.', ephemeral=True)
...
class DropModal(discord.ui.Modal, title="Причина отказа"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            await self.user.send(f"Ваш запрос на получение ролей был отклонён. Причина: {self.reason.value}")
            ...
            # Подтверждение
        await interaction.response.send_message(f"Запрос от {self.user.display_name} отклонён!", ephemeral=True)
...
class ButtonView(discord.ui.View):
    ...
    @discord.ui.button(label="Получить роли", custom_id="register_button", style=discord.ButtonStyle.red)
    async def registerbtn(self, interaction: discord.Interaction, button: discord.ui.Button):
...
class DropButton(discord.ui.Button):
    def __init__(self, embed: discord.Embed, user: discord.User):
        super().__init__(label="Отклонить", style=discord.ButtonStyle.red, custom_id="drop_button")
        self.embed = embed
        self.user = user

    async def callback(self, interaction: discord.Interaction):
        await interaction.response.send_modal(DropModal(self.embed, self.user, self.view))


class DoneButton(discord.ui.Button):
    def __init__(self, embed: discord.Embed, user: discord.User):
        super().__init__(label="Выполнено", style=discord.ButtonStyle.green, custom_id="done_button")
        self.embed = embed
        self.user = user
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        self.embed.set_footer(text=f"Запрос выполнен пользователем {interaction.user.display_name}")
        ...
        await interaction.response.send_message(f"Запрос от {self.user.display_name} выполнен!", ephemeral=True)
